...
public class Utils extends RecipeProvider {
    ...
    public Utils(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> provider) {
        super(packOutput, provider);
    }

    public static void generateColor(RecipeOutput consumer, Block ingredient, Item dye, Block result, String group) {
    ...
    public static void generate(RecipeOutput consumer, Block ingredient, Block result, String group) {
    ...
    public static void generateSmelting(RecipeOutput consumer, @Nullable String group, Item ingredient, Item result, RecipeSerializer<? extends AbstractCookingRecipe> recipeType, float xp) {
    ...
    public static void generateSmelting(RecipeOutput consumer, @Nullable String group, Item ingredient, Item result, RecipeSerializer<? extends AbstractCookingRecipe> recipeType, float xp, int cookingTime) {
        String recipeName;
        ...
        if (group != null) {
            if (recipeType == RecipeSerializer.BLASTING_RECIPE) {
                SimpleCookingRecipeBuilder.blasting(Ingredient.of(ingredient), RecipeCategory.MISC, result, xp, cookingTime)
                        .group(group.toLowerCase())
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer, new ResourceLocation("JBR".toLowerCase(), getItemName(result) + recipeName + "_" + getItemName(ingredient)));
            } else if (recipeType == RecipeSerializer.SMOKING_RECIPE) {
                SimpleCookingRecipeBuilder.smoking(Ingredient.of(ingredient), RecipeCategory.MISC, result, xp, cookingTime)
                        .group(group.toLowerCase())
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer, new ResourceLocation("JBR".toLowerCase(), getItemName(result) + recipeName + "_" + getItemName(ingredient)));
            } else {
                SimpleCookingRecipeBuilder.smelting(Ingredient.of(ingredient), RecipeCategory.MISC, result, xp, cookingTime)
                        .group(group.toLowerCase())
                        .unlockedBy(getHasName(ingredient), has(ingredient))
                        .save(consumer, new ResourceLocation("JBR".toLowerCase(), getItemName(result) + recipeName + "_" + getItemName(ingredient)));
            }
    ...
    public static void generateStonecutting(RecipeOutput consumer, Item ingredient, Item Result, int count) {
    ...
    public static void generateStonecutting(RecipeOutput consumer, Item ingredient, Item Result, int count, RecipeCategory recipeCategory) {
        SingleItemRecipeBuilder.stonecutting(Ingredient.of(ingredient), recipeCategory, Result, count)
                .unlockedBy(getHasName(ingredient), has(ingredient))
                .save(consumer, new ResourceLocation("JBR".toLowerCase(), getConversionRecipeName(Result, ingredient) + "_stonecutting"));
    }

    public static void generate(RecipeOutput consumer, Item ingredient, Item result, String groupName) {
    ...
    @Override
    protected void buildRecipes(RecipeOutput p_251297_) {
    }
}
